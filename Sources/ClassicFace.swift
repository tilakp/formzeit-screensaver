...
/// The original Braun BC12-inspired dial, kept as the "Classic" face (§12
/// decision: cheap to keep, protects users who liked the original look).
/// Unchanged from v1 except that its accent color is now resolved by the
/// caller (`FormzeitRenderer`) through the shared v2 `Accent`/`DielLighting`
/// system rather than the old fixed swatch list, so Classic's second hand
/// still tracks Adaptive/World/Accent choices made in the v2 settings sheet.
enum ClassicFace {

    static func render(context: CGContext, bounds: CGRect, now: Date, elapsedRunTime: TimeInterval, isPreview: Bool,
                        defaults: FormzeitDefaults, accent: NSColor) {
        renderFace(context: context, bounds: bounds, now: now, elapsedRunTime: elapsedRunTime, isPreview: isPreview, defaults: defaults)
        renderHands(context: context, bounds: bounds, now: now, elapsedRunTime: elapsedRunTime, isPreview: isPreview, defaults: defaults, accent: accent)
    }
    ...
    static func renderHands(context: CGContext, bounds: CGRect, now: Date, elapsedRunTime: TimeInterval, isPreview: Bool,
        ...
        drawHands(context: context, center: center, radius: radius, lighting: lighting, time: time,
                   movement: defaults.movement, accent: accent, use24Hour: defaults.use24Hour)
    ...
    static func wallClock(_ date: Date) -> WallClock {
    ...
    /// line-height box. See v1 doc history for the full reasoning: metric-box
    /// centering plants old-style figures at different heights, and
    /// `NSAttributedString.draw(at:)` uses a different coordinate origin than
    /// `CTLineGetBoundsWithOptions` measures in, so drawing goes through
    /// `CTLineDraw` with an explicit baseline-relative `textPosition`.
    ...
    ///  - digital (displayed "Sweep"): perfectly continuous sweep
